...
namespace BleExample.Utils
{
    ...
    public static class GattServiceUuidHelper
    {
        ...
        public static string Name(this GattDeviceService service)
        {
            var shortId = BluetoothUuidHelper.TryGetShortId(service.Uuid);
            if (shortId.HasValue
                && Enum.TryParse(shortId.Value.ToString(), out GattServiceUuid name)
                && Enum.IsDefined(name))
            {
                return name.ToString();
            }
            return GattServiceUuid.Unknown.ToString();
        }

        public static bool IsReadOnly(Guid uuid)
        {
            if (GattServiceUuids.DeviceInformation == uuid 
                || GattServiceUuids.GenericAttribute == uuid 
                || GattServiceUuids.GenericAccess == uuid 
                || GattServiceUuids.ScanParameters == uuid)
            {
                return true;
            }
    ...
    public static class GattCharacteristicUuidHelper
    {
        ...
        /// <param name="uuid"></param>
        /// <returns>Name of the UUID</returns>
        public static string Name(this GattCharacteristic characteristic)
        {
            var shortId = BluetoothUuidHelper.TryGetShortId(characteristic.Uuid);
            if (shortId.HasValue
                && Enum.TryParse(shortId.Value.ToString(), out GattCharacteristicUuid name)
                && Enum.IsDefined(name))
            {
                return name.ToString();
            }
            return GattCharacteristicUuid.Unknown.ToString();
        }
    }
    ...
    public static class GattDescriptorUuidHelper
    {
        ...
        /// <param name="uuid"></param>
        /// <returns>Name of the UUID</returns>
        public static string Name(this GattDescriptor descriptor)
        {
            var shortId = BluetoothUuidHelper.TryGetShortId(descriptor.Uuid);
            if (shortId.HasValue
                && Enum.TryParse(shortId.Value.ToString(), out GattDescriptorUuid name)
                && Enum.IsDefined(name))
            {
                return name.ToString();
            }
            return GattDescriptorUuid.Unknown.ToString();
        }
    }

    public static class AdvertisementDataFlagHelper
    {
        /// <summary>
        /// Helper function to convert a UUID to a name
        /// </summary>
        /// <param name="uuid"></param>
        /// <returns>Name of the UUID</returns>
        public static string DataTypeName(this BluetoothLEAdvertisementDataSection bluetoothLEAdvertisementDataSection)
        {
            if (Enum.IsDefined((AdvertisementDataTypeFlag)bluetoothLEAdvertisementDataSection.DataType))
            {
                return ((AdvertisementDataTypeFlag)bluetoothLEAdvertisementDataSection.DataType).ToString();
            }
            return AdvertisementDataTypeFlag.Unknown.ToString();
        }
    }

    public enum AdvertisementDataTypeFlag : byte
    {
        Unknown = 0,

        Flags = 0x01,
        Incomplete16bServices = 0x02,
        Complete16bServices = 0x03,
        Incomplete32bServices = 0x04,
        Complete32bServices = 0x05,
        Incomplete128bServices = 0x06,
        Complete128bServices = 0x07,
        ShortLocalName = 0x08,
        CompleteLocalName = 0x09,
        TxPower = 0x0a,
        ServiceSolicitation16b = 0x14,
        ServiceSolicitation32b = 0x1f,
        ServiceSolicitation128b = 0x15,
        ServiceData16b = 0x16,
        ServiceData32b = 0x20,
